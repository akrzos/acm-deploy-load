#!/usr/bin/env bash
# Test consumes all available SNO capacity
set -e
set -o pipefail
...
time ./sno-deploy-load/sno-deploy-load.py --acm-version "${acm_ver}" --test-version "${test_ver}" --hub-version "${hub_ocp}" --sno-version "${sno_ocp}" --wan-emulation "${wan_em}" --snos-per-app ${snos_per_app} -w -i 60 -t int-ztp-${snos_per_app}-${batch}b-${interval_period}i-${iteration} interval -b ${batch} -i ${interval_period} ztp 2>&1 | tee ${log_file}
...
time ./sno-deploy-load/analyze-sno-deploy-time.py ${results_dir} 2>&1 | tee -a ${log_file}
