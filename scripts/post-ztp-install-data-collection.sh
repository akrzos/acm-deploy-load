...
if [ "$1" == "-k" ]; then
  ...
  ls /root/hv-vm/sno/ai-manifest/ | sed 's/-manifests.yml//g' | xargs -I % sh -c "mkdir -p /root/hv-vm/kc/% ;oc get secret %-admin-kubeconfig -n % -o json | jq -r '.data.kubeconfig' | base64 -d > /root/hv-vm/kc/%/kubeconfig"
