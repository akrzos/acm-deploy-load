...
echo
cat phase*/cluster/stats/net-rcv-cluster_no-minio.stats | grep 95% | awk '{print $2}'
echo
echo "Cluster Network XMT 95 percentile"
echo
cat phase*/cluster/stats/net-xmt-cluster_no-minio.stats | grep 95% | awk '{print $2}'

# Double check the columns
echo
echo "MCE PVC max"
echo
ls phase*/resource/stats/pvc-usage.stats | xargs -I % sh -c 'echo "$(cat %)"' | grep -v minio | grep max | awk '{print $3}'

echo
echo "ACM OBS PVC max"
echo
ls phase*/resource/stats/pvc-usage.stats | xargs -I % sh -c 'echo "$(cat %)"' | grep -v minio | grep max | awk '{print $4}'

echo
echo "OpenShift Monitoring PVC max"
echo
ls phase*/resource/stats/pvc-usage.stats | xargs -I % sh -c 'echo "$(cat %)"' | grep -v minio | grep max | awk '{print $5}'

echo
echo "Minio PVC max"
echo
ls phase*/resource/stats/pvc-usage.stats | xargs -I % sh -c 'echo "$(cat %)"' | grep -v minio | grep max | awk '{print $2}'
