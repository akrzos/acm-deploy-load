...
def generate_report(start_time, end_time, deploy_start_time, deploy_end_time, wait_sno_start_time, wait_sno_end_time,
  ...
  sucess_sno_percent = 0
  failed_sno_percent = 0
  sucess_du_percent = 0
  failed_du_percent = 0
  if total_deployed_snos > 0:
    sucess_sno_percent = round(monitor_data["sno_install_completed"] / total_deployed_snos, 3) * 100
    failed_sno_percent = round(100 - sucess_sno_percent, 1)
  if monitor_data["policy_init"] > 0:
    sucess_du_percent = round(monitor_data["policy_compliant"] / monitor_data["policy_init"], 3) * 100
    failed_du_percent = round(100 - sucess_du_percent, 1)
  ...
  with open("{}/report.txt".format(report_dir), "w") as report:
    ...
    log_write(report, " * SNO Successful Percent: {}%".format(sucess_sno_percent))
