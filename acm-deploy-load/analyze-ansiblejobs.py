...
def main():
  ...
  jobs_analyzed = len(aj_data["items"])
  jobs_status = {}
  ...
  for item in aj_data["items"]:
    ...
    if "status" in item["status"]["ansibleJobResult"]:
      ...
      if aj_result_status in jobs_status:
        jobs_status[aj_result_status] += 1
      else:
        jobs_status[aj_result_status] = 1
  ...
  with open(aj_stats_file, "w") as stats_file:
    log_write(stats_file, "Analyzed {} ansiblejobs".format(jobs_analyzed))
    for status in sorted(jobs_status.keys()):
      log_write(stats_file, "AnsibleJobs with {} status: {}".format(status, jobs_status[status]))
    ...
    if len(create_started_durations) > 0:
      log_write(stats_file, "Min: {}".format(np.min(create_started_durations)))
      ...
      log_write(stats_file, "Max: {}".format(np.max(create_started_durations)))
    log_write(stats_file, "#############################################")
    log_write(stats_file, "Started to Finished Duration Stats")
    log_write(stats_file, "Count: {}".format(len(started_finished_durations)))
    if len(create_started_durations) > 0:
      log_write(stats_file, "Min: {}".format(np.min(started_finished_durations)))
      ...
      log_write(stats_file, "Max: {}".format(np.max(started_finished_durations)))
    log_write(stats_file, "#############################################")
    log_write(stats_file, "Complete Duration Stats")
    log_write(stats_file, "Count: {}".format(len(complete_durations)))
    if len(create_started_durations) > 0:
      log_write(stats_file, "Min: {}".format(np.min(complete_durations)))
      ...
      log_write(stats_file, "Max: {}".format(np.max(complete_durations)))
