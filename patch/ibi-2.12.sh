...
oc get deploy -n multicluster-engine image-based-install-operator -o json |  jq '.spec.template.spec.containers[] |= (select(.name=="manager").image = "e38-h01-000-r650.rdu2.scalelab.redhat.com:5000/ibio/image-based-install-operator:cabundle")' | oc replace -f -
oc get deploy -n multicluster-engine image-based-install-operator -o json |  jq '.spec.template.spec.containers[] |= (select(.name=="server").image = "e38-h01-000-r650.rdu2.scalelab.redhat.com:5000/ibio/image-based-install-operator:cabundle")' | oc replace -f -
